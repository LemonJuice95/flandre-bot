...
public class EnableFunctionCommand extends GroupCommandRunner {
    ...
    public boolean validate(Message command) {
        ...
        return message.startsWith(CQCodeUtils.at(command.selfId) + "/启用功能");
    }
    ...
    public void apply(Message command) {
        ...
        if(FunctionNameManager.GROUP_FUNCTIONS.containsKey(funcMessage)) {
            ...
            SendingUtils.sendGroupText(command.groupId, CQCodeUtils.reply(command.messageId) + "已启用功能 " + funcMessage);
        } else {
